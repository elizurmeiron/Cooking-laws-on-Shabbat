...
class FlowchartConnector {
    ...
    drawConnections() {
        ...
        } else if (this.connectionType === 'hanachah') {
            // Connections for placement flowchart (slide 15)
            const connections = [
                // Question 1 to answers
                { from: 'question1b', to: 'q1b-yes', color: 'var(--category-gradient-start)' },
                { from: 'question1b', to: 'q1b-no', color: 'var(--category-error-start)' },

                // Question 2 to answers (liquid/dry)
                { from: 'question2b', to: 'q2b-lach', color: 'var(--link-color)' },
                { from: 'question2b', to: 'q2b-yavesh', color: '#ed8936' },

                // Question 3 (how to place dry) to answers
                { from: 'question3b', to: 'q3b-yashir', color: 'var(--category-error-start)' },
                { from: 'question3b', to: 'q3b-mutarot', color: 'var(--category-gradient-start)' }
            ];

            connections.forEach(conn => {
                this.drawConnection(conn.from, conn.to, conn.color);
            });
        } else if (this.connectionType === 'hatmanah-erev') {
            // Connections for insulation before Shabbat flowchart (slide 19)
            const connections = [
                // Question 1 - what to cover with
                { from: 'question1d', to: 'q1d-shomer', color: 'var(--category-gradient-start)' },
                { from: 'question1d', to: 'q1d-mosif', color: 'var(--category-error-start)' }
            ];

            connections.forEach(conn => {
                this.drawConnection(conn.from, conn.to, conn.color);
            });
        } else if (this.connectionType === 'hatmanah-shabbat') {
            // Connections for insulation on Shabbat flowchart (slide 20)
            const connections = [
                // Question 1 - is the pot visible
                { from: 'question1e', to: 'q1e-galuy', color: 'var(--category-gradient-start)' },
                { from: 'question1e', to: 'q1e-mechus', color: 'var(--category-error-start)' }
            ];

            connections.forEach(conn => {
                this.drawConnection(conn.from, conn.to, conn.color);
            });
        } else if (this.connectionType === 'magis-a') {
            // Connections for stirring flowchart A (cooked completely)
            const connections = [
                { from: 'question-magis-a', to: 'qa-yes', color: 'var(--category-error-start)' },
                { from: 'question-magis-a', to: 'qa-no', color: 'var(--category-gradient-start)' }
            ];

            connections.forEach(conn => {
                this.drawConnection(conn.from, conn.to, conn.color);
            });
        } else if (this.connectionType === 'magis-b') {
            // Connections for stirring flowchart B (not cooked completely)
            const connections = [
                { from: 'question-magis-b', to: 'qb-yes', color: '#d32f2f' },
                { from: 'question-magis-b', to: 'qb-no', color: '#f57c00' }
            ];
...
/**
 * Global function to redraw the flowchart for the currently displayed slide
 * Used when theme changes to update flowchart colors
 */
function redrawCurrentFlowchart() {
    // Determine which slide is currently active and redraw its flowchart
    ...
    // Redraw flowchart based on current slide
    if (currentIndex === 13) { // Slide 14 - delay flowchart
        new FlowchartConnector('connectionCanvas', 'flowchart', 'default');
    } else if (currentIndex === 15) { // Slide 16 - placement flowchart
        new FlowchartConnector('connectionCanvas2', 'flowchart2', 'hanachah');
    } else if (currentIndex === 17) { // Slide 18 - return flowchart
        new FlowchartConnector('connectionCanvas3', 'flowchart3', 'hachzarah');
    } else if (currentIndex === 19) { // Slide 20 - insulation flowcharts
    ...
    } else if (currentIndex === 21) { // Slide 22 - stirring flowcharts
